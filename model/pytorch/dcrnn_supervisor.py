...
#         self._logger.info("Loaded model at {}".format(self._epoch_num))
    def save_model(self, epoch):
        if not os.path.exists('week15_shuffled/models_seed%d' % self.random_seed):
            os.makedirs('week15_shuffled/models_seed%d' % self.random_seed)

        config = dict(self._kwargs)
        config['model_state_dict'] = self.dcrnn_model.state_dict()
        config['epoch'] = epoch
        torch.save(config, 'week15_shuffled/models_seed%d/epo%d.tar' % (self.random_seed, epoch))
        self._logger.info("Saved model at {}".format(epoch))
        return 'week15_shuffled/models_seed%d/epo%d.tar' % (self.random_seed, epoch)

    def load_model(self):
        self._setup_graph()
        assert os.path.exists('week15_shuffled/models_seed%d/epo%d.tar' % (self.random_seed, self._epoch_num)), 'Weights at epoch %d not found' % self._epoch_num
        checkpoint = torch.load('week15_shuffled/models_seed%d/epo%d.tar' % (self.random_seed, self._epoch_num), map_location='cpu')
        self.dcrnn_model.load_state_dict(checkpoint['model_state_dict'])
        self._logger.info("Loaded model at {}".format(self._epoch_num))
